...
const Home = () => {
    const imageUrl = 'https://images.unsplash.com/photo-1550071593-fd1bdaf1f93c?crop=entropy&cs=tinysrgb&fit=crop&fm=jpg&h=480&ixid=MnwxfDB8MXxyYW5kb218MHx8Y29tcHV0ZXJ8fHx8fHwxNjk5NDM4NDg2&ixlib=rb-4.0.3&q=80&utm_campaign=api-credit&utm_medium=referral&utm_source=unsplash_source&w=720'

    const listItems = ['Rendering Multiple Elements', 'Rendering Adjacent Elements', 'Elements Inside a Component', 'Rendering Multiple Elements', 'Rendering Adjacent Elements', 'Elements Inside a Component']
        ...
        <section className='h-full flex flex-col justify-between gap-8 text-gray-900'>
            <div className='lg:grid lg:grid-cols-5 lg:justify-between lg:items-center lg:gap-4 p-8'>
                ...
                <div className='lg:col-span-2 overflow-hidden rounded-md md:mx-12 lg:m-0'>
                    <img src={imageUrl} alt='banner' className='rounded-md hover:scale-105 ease-in-out duration-500' />
                </div>
            </div>
            ...
            <div className='p-8'>
                <h1 className='text-sky-700 md:text-3xl text-2xl uppercase text-center'>Lorem ipsum dolor sit.</h1>
                <div className='md:grid lg:grid-cols-3 md:grid-cols-2 flex flex-col items-center justify-center md:m-8 my-4'>
                    {listItems.map((listItem, index) =>
                        <span className='bg-slate-200 w-fit md:p-2 md:m-2 p-1 m-1 md:text-xl text-base rounded-md inline-flex items-center gap-2 cursor-pointer hover:scale-105 ease-in-out duration-300'
                            key={index}>
